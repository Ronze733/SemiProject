window.addEventListener('load', () => {
  ...
  nicknameInput.addEventListener('input', function() {
    ...
    const nicknameRegex = /^[a-zA-Z가-힣]{4,12}$/;
    const nicknameErrorMessage = document.getElementById('nickname-error-message');

    if (!nicknameRegex.test(nicknameValue)) {
      nicknameInput.setCustomValidity('닉네임은 한글과 영문만 허용되며 4~12자로 입력해야 합니다.');
      nicknameErrorMessage.textContent = '닉네임은 한글과 영문만 허용되며 4~12자로 입력해야 합니다.';
    } else {
